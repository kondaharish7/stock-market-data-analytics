...
for index, row in df_stocks_list.iterrows():
    ...
    stock_name = row.loc['name'];print(f"Saving html file for {stock_name}, ", end="");log_time=datetime.now()
    try:
        stock_url_response = requests.get(stock_url, timeout=2)
        stock_url_html_page = BeautifulSoup(stock_url_response.text, 'html.parser')
        stocks_html_file_key = f"data/stocks_html_files/{stock_name.replace(" ", "_")}.html"
        ...
        print(f"Timed Out, elapsed: {datetime.now() - log_time}")
    else:
